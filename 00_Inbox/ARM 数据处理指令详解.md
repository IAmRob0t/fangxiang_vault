...
1.  从内存中**加载** `a` 的值到 CPU 的一个寄存器（如 `R0`）。
2.  从内存中**加载** `b` 的值到 CPU 的另一个寄存器（如 `R1`）。
3.  在 CPU 内部**处理**（计算）`R0 * R1`，结果存放在一个寄存器中（如 `R0`）。
4.  将计算结果从寄存器 `R0` **存储**回内存中的 `a` 所在地址。

![RISC架构的Load-Store模型](attachments/06_数据处理指令/RISC架构的Load-Store模型.png)
...
![数据处理指令通用语法格式](attachments/06_数据处理指令/数据处理指令通用语法格式.png)
...
![条件码后缀表](attachments/06_数据处理指令/条件码后缀表.png)

### 2.2 APSR 标志位

`{S}` 后缀会更新 APSR 中的条件标志位，这些标志位是实现条件判断和分支的基础。

![程序状态寄存器中的标志位](attachments/06_数据处理指令/程序状态寄存器中的标志位.png)
...
![数据处理指令集速查表](attachments/06_数据处理指令/数据处理指令集速查表.png)
