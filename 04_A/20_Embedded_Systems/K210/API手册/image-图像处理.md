...
-  **关键参数**： `thresholds` 必须是[[01_Python基础#`list` 和 `tuple`|元组列表]] 。 `[(lo, hi), (lo, hi), ..., (lo, hi)]` 定义你想追踪的颜色范围。对于灰度图像，每个元组需要包含两个值 - 最小灰度值和最大灰度值。仅考虑落在这些阈值之间的像素区域。对于RGB565图像，每个元组需要有六个值 `(l_lo，l_hi，a_lo，a_hi，b_lo，b_hi)` - 分别是LAB L，A和B通道的最小值和最大值。为方便使用，此功能将自动修复交换的最小值和最大值。此外，如果元组大于六个值，则忽略其余值。相反，如果元组太短，则假定其余阈值处于最大范围。
...
-  **性能关键**： `x_stride` 是查找某色块时需要跳过的x像素的数量。找到色块后，直线填充算法将精确像素。若已知色块较大，可增加 `x_stride` 来提高查找色块的速度。`y_stride` 是查找某色块时需要跳过的y像素的数量。找到色块后，直线填充算法将精确像素。若已知色块较大，可增加 `y_stride` 来提高查找色块的速度。
...
| 参数名              | 类型        | 默认值   | 说人话解释                 | 注意事项                                                       |
| ---------------- | --------- | ----- | --------------------- | ---------------------------------------------------------- |
| thresholds       | 元组列表      | 必填    | 你要找的颜色范围（LAB或灰度）      | 红色示例：[(30,60,15,50,15,50)] 灰度示例：[(200,255)]                |
| invert           | bool      | False | 反选颜色：找阈值范围外的颜色        | 当背景颜色统一时开启效果更好                                             |
| roi              | (x,y,w,h) | 全图    | 只检测画面中的某个区域           | ⚠️区域必须64字节对齐（如设置成(0,0,128,128)）                            |
| x_stride         | int       | 2     | 横向扫描步长（越大检测越快但容易漏小色块） | 检测大色块时建议设为8                                                |
| y_stride         | int       | 1     | 纵向扫描步长                | 通常保持1，对高大物体可设为2                                            |
| area_threshold   | int       | 10    | 过滤面积小于该值的色块（单位：像素面积）  | 实测面积=包围盒的w*h，非实际像素数                                        |
| pixels_threshold | int       | 10    | 过滤实际像素数小于该值的色块        | 比area_threshold更精准                                         |
| merge            | bool      | False | 合并重叠的色块               | 适合处理分散的同色区域                                                |
| margin           | int       | 0     | 合并时的边界扩展像素            | 设10表示两个色块间距<10像素就合并                                        |
| threshold_cb     | 函数        | None  | 自定义过滤条件（高级用法）         | 回调函数示例：`lambda blob: blob.w()>50` 只保留宽度>50的色块              |
| merge_cb         | 函数        | None  | 自定义合并条件（高级用法）         | 回调函数示例：`lambda b1,b2: abs(b1.code()-b2.code())==0` 只合并同色色块 |
...
### 18.1. `image.SEARCH_EX`

详尽的模板匹配搜索。

### 18.2. `image.SEARCH_DS`

更快的模板匹配搜索。

### 18.3. `image.EDGE_CANNY`

使用Canny边缘检测算法对图像进行边缘检测。

### 18.4. `image.EDGE_SIMPLE`

使用阈值高通滤波算法对图像进行边缘检测。

### 18.5. `image.CORNER_FAST`

用于ORB键点的高速低准确率角点检测算法

#### 18.5.1. `image.CORNER_AGAST`
...
### 18.10. `image.TAG36H11`

TAG36H11标签群的位掩码枚举。用于AprilTags。

### 18.11. `image.ARTOOLKIT`
