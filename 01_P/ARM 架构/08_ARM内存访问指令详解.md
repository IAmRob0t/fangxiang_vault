...
![](attachments/08_ARM内存访问指令详解/file-20251119160028551.png)
...
![](attachments/08_ARM内存访问指令详解/file-20251119160028552.png)
...
![file-20251119160028551 1](attachments/08_ARM内存访问指令详解/file-20251119160028551%201.png)
...
![file-20251119160028555](attachments/08_ARM内存访问指令详解/file-20251119160028555.png)
...
![file-20251119160028552 1](attachments/08_ARM内存访问指令详解/file-20251119160028552%201.png)
...
**栈指针 (SP)** 是CPU中的一个专用寄存器，它始终指向栈的顶部（或底部，取决于栈的增长方向）。栈是内存中一块用于临时存储数据的区域，例如函数调用时的局部变量、函数参数和返回地址。SP寄存器在数据入栈（push）或出栈（pop）时会自动（或通过汇编指令手动）更新，是管理栈操作的核心。
...
![file-20251119160028556](attachments/08_ARM内存访问指令详解/file-20251119160028556.png)
