...
*   **`LDR` (Load Register)**：顾名思义，“Load”是“加载”的意思，它用于将数据**从内存加载到寄存器**。可以理解为把内存中的数据“取出来”，放到CPU内部的寄存器中进行处理。
*   **`STR` (Store Register)**：顾名思义，“Store”是“存储”的意思，它用于将数据**从寄存器存储到内存**。可以理解为把CPU寄存器中处理好的数据“存回去”，放到内存中。
...
**LDR 与 LDRB 的区别：** 在于它们加载数据的大小：`LDR` 加载32位，`LDRB` 加载8位。
...
![](attachments/08_ARM内存访问指令详解/file-20251117233850486.png)
...
STR		R2, [R0], #0X20         ; R2的值存到R0所示地址 (0x20108), R0更新为0x20008 + 0x20 = 0x20028
MOV		R2, #0x34               ; R2 = 0x34
STR		R2, [R0]                ; R2的值存到R0所示地址 (0x20028)
LDR		R3, [R0], +R1, LSL #1   ; R3的值等于R0+(R1<<1)所示地址上的值 (0x20028 + (0x10<<1) = 0x20028 + 0x20 = 0x20048)
```
...
![file-20251117233850486 1](attachments/08_ARM内存访问指令详解/file-20251117233850486%201.png)
...
![file-20251117233850487](attachments/08_ARM内存访问指令详解/file-20251117233850487.png)
...
![file-20251117233850487 1](attachments/08_ARM内存访问指令详解/file-20251117233850487%201.png)
...
![file-20251117233850488](attachments/08_ARM内存访问指令详解/file-20251117233850488.png)
