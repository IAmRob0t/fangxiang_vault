...
*   **`LDR` (Load Register)**：用于从内存中加载一个**字（Word）**到寄存器。在ARM架构中，一个字通常是32位。
*   **`LDRB` (Load Register Byte)**：用于从内存中加载一个**字节（Byte）**到寄存器。一个字节是8位。当将一个字节加载到32位寄存器时，通常会将该字节进行零扩展（即高24位填充0），以填充整个寄存器。

**主要区别**在于它们加载数据的大小：`LDR` 加载32位，`LDRB` 加载8位。
